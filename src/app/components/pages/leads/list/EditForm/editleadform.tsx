...
const EditLeadForm: React.FC<{
  ...
  return formatted;
      ...
      setValue(
        "pickupDateTime",
        formatDateTimeForInput(initialData.pickupDateTime),
      );
      setValue(
        "dropDateTime",
        formatDateTimeForInput(initialData.dropDateTime),
      );
  ...
  // Format datetime for submit - ensure we preserve time properly
  const formatDateTimeForSubmit = (dateStr: string | undefined) => {
    if (!dateStr) return undefined;

    // The datetime-local input gives us YYYY-MM-DDTHH:MM format when user changes time
    // We need to convert T to space and ensure seconds are included
    ...
    if (parts.length >= 2) {
      ...
      if (timePart && timePart.includes(":")) {
        const timeComponents = timePart.split(":");
        // If only hours and minutes (2 parts), add seconds
    ...
    console.log("formatDateTimeForSubmit:", dateStr, "->", formatted);
    return formatted;
  };
